...
<body>
	...
	<form class="applyForm" name="form" method="POST"
		action="<c:url value='/animal/register'/>"
		...
		<script>
			$(document).ready(
					function() {
						...
						$("select[id='animal_type']").on(
								"change",
								function() {
									var option = $(
											"#animal_type option:selected")
											.val();
									var subSelName = '';
									if (option == "개") {
		...
		<!-- 과: type (개, 고양이, 기타) -->
		과: <select name="animal_type" id="animal_type">
		</select> <br>
		<br>
		<!-- 종: species (포메라니안, 요크셔테리어, 치와와) -->
		종: <select name="species" id="species" style="">
		</select> <br>
		<br>
		<!-- 성별: gender  -->
		...
		<!-- 특이사항: etc -->
		특이사항:
		<textarea id="etc" name="etc" rows=10 " cols="30"
			placeholder="특이사항을 입력하시오."></textarea>
		<br>
